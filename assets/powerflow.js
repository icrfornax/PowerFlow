...
(function () {
  ...
  function redispatchAbschnitt(von, bis, netzlast, lasttage) {
    ...
    if (K && K.monate && K.monate.length) {
      ...
      if (maxK > 0) {
        /* Der Massstab nennt BEIDE Groessen, die ihn ausschoepfen. "Balken bis
           392 Mio." war falsch: so hoch ist kein Balken, so hoch ist die
           hoechste VORJAHRESMARKE. Wer das liest und die Balken ansieht, haelt
           die Grafik fuer kaputt. */
        kkasten.appendChild(el("p", { "class": "pf-kosten-massstab",
          text: "Balken und Vorjahresmarken teilen einen Maßstab, er reicht bis "
            + eur(maxK) + " im Monat"
            + (voll && stellen.length > summenStellen.length
                ? " · die Monate des Zeitraums voll, die übrigen gedämpft"
                : "")
            + " · die gestrichelte Fläche dahinter ist derselbe Monat ein Jahr "
            + "früher" }));
        ...
        stellen.forEach(function (i) {
          ...
          var vj = vorjahr(i);
          if (vj !== null && maxK) {
            // Hoehe statt bottom: die Flaeche steht auf der Grundlinie und
            // endet auf dem Vorjahreswert.
            sp.appendChild(el("i", { "class": "pf-kosten-vorjahr",
              style: "height:" + (Math.abs(vj) / maxK * 100).toFixed(1) + "%;" }));
          }
  ...
  /* ZURUECKGENOMMEN am 15.09.2026: der Mehrjahresvergleich als Block auf der
     Seite. Er ist gebaut, angesehen und von Immo verworfen worden -- zu
     haesslich, und die Aussage trug nicht. Vier Reihen aus je zwoelf Balken
     sind viel Flaeche fuer wenig: dass die Netzlast ruhig ist und die
     Residuallast springt, steht in zwei Zahlen schneller da als in 48 Saeulen.

     Die RECHNUNG bleibt und ist keine Baustelle: mehrjahresreihen() und
     mehrjahresstreuung() stehen darueber und bedienen den CSV-Abzug
     "Derselbe Zeitraum in allen Jahren", der dieselbe Frage beantwortet und
     seit dem 14.09.2026 unveraendert steht.

     WER DEN BLOCK WIEDER BAUEN WILL, liest vorher
     docs/beleg-mehrjahresvergleich.md -- dort steht, was daran nicht
     getragen hat. */
